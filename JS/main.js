...
const translations = {
  ...
  de: {
    ...
    PortfolioDesc: "Die Website, auf der Sie sich gerade befinden, ist ebenfalls eines meiner Projekte. Die Website wurde mit HTML, TailwindCSS and JavaScript erstellt.",
  ...
  function loop() {
    if (!isDeleting) {
      if (index < text.length) {
        ...
        index++;
        typeTimeout = setTimeout(loop, typeSpeed);
      } else {
      ...
      if (index > 0) {
        ...
        index--;
        typeTimeout = setTimeout(loop, eraseSpeed);
      } else {
        isDeleting = false;
        typeTimeout = setTimeout(loop, holdAfterErase);
      }
    }
  }
...
function setLanguage(lang) {
  ...
  // Restart typewriter
  startTypewriterLoop(translations[lang].heroTitle);
}
